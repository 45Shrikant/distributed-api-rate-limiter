  ...
  return (
    <div className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800 backdrop-blur-sm">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-base font-bold text-white">Recent API Requests</h3>
          <p className="text-xs text-slate-400">Real-time audit stream logged to MongoDB</p>
        </div>
      </div>
        ...
        <table className="w-full text-left text-xs font-mono">
          <thead>
            <tr className="border-b border-slate-800 text-slate-400 uppercase tracking-wider text-[11px]">
              ...
              <th className="pb-3 px-3">Instance</th>
              ...
              requests.map((req, idx) => (
                <tr key={req._id || idx} className="hover:bg-slate-800/40 transition-colors">
                  <td className="py-2.5 px-3">
                    <span className={`px-2 py-0.5 rounded border text-[11px] font-bold ${getStatusBadge(req.statusCode)}`}>
                      {req.statusCode}
                    </span>
                  </td>
                  <td className="py-2.5 px-3">
                    <span className={`px-2 py-0.5 rounded border text-[10px] font-bold ${getMethodBadge(req.method)}`}>
                      {req.method}
                    </span>
                  </td>
                  <td className="py-2.5 px-3 font-medium text-slate-200">{req.endpoint}</td>
                  <td className="py-2.5 px-3 text-slate-400">{req.responseTime}ms</td>
                  <td className="py-2.5 px-3 text-slate-400">{req.ip}</td>
                  <td className="py-2.5 px-3 text-cyan-400 font-semibold">{req.serverInstance || 'server-1'}</td>
